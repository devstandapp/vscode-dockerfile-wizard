...
export const saveEnabled: Readable<boolean> = derived([baseImage, dockerIgnoreText], ([$baseImage, $dockerIgnoreText]) => {
	return $baseImage !== undefined && $dockerIgnoreText.length > 0
})
...
formResult.subscribe(
	debounce(100, ($formResult: AssemblyFormResult) => {
		extension.formResultChanged($formResult)
	})
)
