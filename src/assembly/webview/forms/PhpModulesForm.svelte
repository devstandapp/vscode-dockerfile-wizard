<script lang="ts">
	...
	import { phpModulesManuallyAdded, phpModulesManuallyAddedToView, phpModulesFromPackageJsonExcluded, phpModulesFromPackageJsonCbxIndeterminate, phpModulesFromRequiredPackagesExcluded, phpModulesFromRequiredPackagesCbxIndeterminate, phpModulesRecommendedByFrameworkExcluded, phpModulesRecommendedByFrameworkCbxIndeterminate } from '../stores/form.store'
	...
	{#if $preferenceExpanded}
		...
		<div class="flex gx-2">
			<div>
				<label class="input-checkbox cursor-not-allowed">
					<input type="checkbox" checked disabled id="phpModulesBuiltinCbx"> {@html iconCheckbox}
				</label>
			</div>
			<div class="sans-lh">
				<label class="cursor-pointer" for="phpModulesBuiltinCbx">
					Builtin modules preinstalled with PHP {$baseImage.phpVersion} in {$baseImage.baseImageName} {$baseImage.baseImageTag}
				</label>
				<div>
			...
			<div class="flex gx-2">
				<div>
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox" id="phpModulesFromPackageJsonCbx"
							bind:checked={$phpModulesFromPackageJsonCbx}
							indeterminate={$phpModulesFromPackageJsonCbxIndeterminate}
						>{@html iconCheckbox}
					</label>
				</div>
				<div class="sans-lh">
					<label class="cursor-pointer" for="phpModulesFromPackageJsonCbx">
						Modules explicitly mentioned in <span class="mono-family">require</span> portion of <span class="mono-family">package.json</span>
					</label>
					<div>
						<div class="inline-flex flex-wrap gx-2">
							{#each $phpReport.phpModulesFromPackageJson as phpModuleName}
								<span class="mono-family"
		...
		{#if $phpReport.phpModulesFromRequiredPackages.length}
			<div class="flex gx-2">
				<div>
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox" id="phpModulesFromRequiredPackages"
							bind:checked={$phpModulesFromRequiredPackagesCbx}
							indeterminate={$phpModulesFromRequiredPackagesCbxIndeterminate}
						>{@html iconCheckbox}
					</label>
				</div>
				<div class="sans-lh">
					...
					<div>
						<div class="inline-flex flex-wrap gx-2">
							{#each $phpReport.phpModulesFromRequiredPackages as phpModule}
								<span class="mono-family decoration-error decoration-2"
		...
		{#if $phpReport.phpModulesRecommendedByFramework.length}
			<div class="flex gx-2">
				<div>
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox" id="phpModulesRecommendedByFramework"
							bind:checked={$phpModulesRecommendedByFrameworkCbx}
							indeterminate={$phpModulesRecommendedByFrameworkCbxIndeterminate}
						>{@html iconCheckbox}
					</label>
				</div>
				<div class="sans-lh">
					<label class="cursor-pointer" for="phpModulesRecommendedByFramework">
						Modules recommended for the framework
					</label>
					<div>
						<div class="inline-flex flex-wrap gx-2">
							{#each $phpReport.phpModulesRecommendedByFramework as phpModule}
								<span class="mono-family decoration-warning decoration-2"
									class:color-dimmed={$baseImage.phpModulesBuiltin.includes(phpModule.module)}
		...
		{#if ! $expandedCheckedPhpModules && $phpModulesManuallyAddedToView.length}
			<div class="flex gx-2">
				<div>
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox" id="phpModulesManuallyAdded"
							checked={$phpModulesManuallyAdded.length > 0}
							on:input={(event)=>{ if (! event.currentTarget.checked) { phpModulesManuallyAdded.set([]) } }}
						>{@html iconCheckbox}
					</label>
				</div>
				<div class="sans-lh">
					<label class="cursor-pointer" for="phpModulesManuallyAdded">
			...
			{#each $phpModulesAll as phpModuleName }
				...
				<div class="inline-flex items-center">
					...
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox"
							value={phpModuleName}
							checked={$phpModulesChecked.includes(phpModuleName)}
							on:click={onSingleModuleClick}
							id={`cbx-${phpModuleName}`}
							disabled={isBuiltin}
							> {@html iconCheckbox}
					</label>

					<label class="mono-family cursor-pointer pl-2"
						class:color-dimmed={isBuiltin}
						for={`cbx-${phpModuleName}`}
					>{phpModuleName}</label>
					...
					{#if isFromPackageJson || requiredByPackages.length || recommendedByFrameworkNotes.length}
						...
						<span class="icon bold sans-lh-loose cursor-help ml-2"
							class:color-dimmed={isBuiltin}
		...
		<div class="pl-8">
			<a href="##" class="sans-lh color-link select-none focus:outline" on:click={()=>{ $expandedCheckedPhpModules = ! $expandedCheckedPhpModules }}>
...
<style>
	.ModulesColumns { columns: 1; }
	@media (min-width: 400px)  { .ModulesColumns { columns: 2; } }
	@media (min-width: 550px)  { .ModulesColumns { columns: 3; } }
	@media (min-width: 700px)  { .ModulesColumns { columns: 4; } }
