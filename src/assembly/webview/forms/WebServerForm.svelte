...
<div class="flex flex-col gy-4 p-4 bg-preference outline-preference" tabindex="0">
	...
	{#if $preferenceExpanded}
		<div class="color-dimmed sans-lh">
			Currently only
			<a href="https://unit.nginx.org" class="hover:color-link focus:outline underline">Nginx Unit</a>
			is supported. Support for Swoole/Octane is in the roadmap.
		</div>

		<div>
			<div class="mb-2 sans-lh">Listen HTTP traffic on port</div>
			<div class="inline-flex items-center gx-2">
				<input type="number" class="input-text rounded mono-family w-28" bind:value={$serverPort} min="1025" />
				<span class:color-error={$serverPort <= 1024} class:color-dimmed={$serverPort > 1024}>
					unprivileged port (value greater than 1024) is mandatory for
					<span class="cursor-help underline-abbr" title="Container images that can be run without root privileges, providing a more secure and isolated sandbox">rootless images</span>
				</span>
			</div>
		</div>
		...
		<div>
			...
			<div class="inline-flex items-center gx-2">
				<input type="number" class="input-text rounded mono-family w-20" bind:value={$serverRequestSize} min="1" />
		...
		<div class="flex gx-4">
			<div>
				...
				<input type="text" class="input-text rounded mono-family" bind:value={$documentRoot} />
			</div>
			<div>
				<div class="mb-2 sans-lh">Front controller</div>
				<input type="text" class="input-text rounded mono-family" bind:value={$frontController} />
			</div>
		</div>
