	...
	{#if $preferenceExpanded}
		<div class="sans-lh">
			...
			{#if $dockerIgnoreIsExisting === true}
				...
				<span class="mono-family">.dockerignore</span>
				file in your sources.
				<br />
				To prevent accidental bloat you could
				<a
					href="##"
					class="color-link select-none outline-none"
					on:click={() => {
						appendDockerIgnoreProposedLines()
					}}
				>
					append contents of all <span class="mono-family">.gitignore</span>
					files
				</a>
				found in your sources.
			{:else if $dockerIgnoreIsExisting === false}
				<br />
				The following text is generated from contents of all
				<span class="mono-family">.gitignore</span>
				files found in your sources.
			{/if}
		</div>

		<div>
			<textarea class="input-text rounded mono-family lh-normal w-full" rows="2" bind:this={textareaElement} bind:value={$dockerIgnoreText} />
		</div>
	{/if}
</div>
