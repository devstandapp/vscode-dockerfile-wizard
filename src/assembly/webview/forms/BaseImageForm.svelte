...
<div class="flex flex-col gy-5 p-4 bg-preference outline-preference" tabindex="0">
	...
	{#if $preferenceExpanded}
			...
			{#each phpVersions as _phpVersion (_phpVersion)}
				<div class="flex items-center -mt-1">
					<label class="input-radio cursor-pointer">
						<input type="radio" id={`phpVersion_${_phpVersion}`} bind:group={$phpVersion} value={_phpVersion} />
						{@html iconRadio}
					</label>
				...
				<div class="flex g-4">
					{#each $baseImageNames as _baseImageName (_baseImageName)}
						...
						<div class="flex items-center">
							<label class="input-radio cursor-pointer">
								<input type="radio" id={`baseImageName_${_baseImageName}`} bind:group={$baseImageName} value={_baseImageName} />
								{@html iconRadio}
							</label>
			...
			<div>
				...
				<div class="flex g-4">
					{#each $baseImageTags as _baseImageTag (_baseImageTag)}
						<div class="flex items-center">
							<label class="input-radio cursor-pointer">
								<input type="radio" id={`baseImageTag_${_baseImageTag}`} bind:group={$baseImageTag} value={_baseImageTag} />
								{@html iconRadio}
							</label>
		...
		<div class="color-dimmed sans-lh">
			Currently only <a href="https://alpinelinux.org" class="hover:color-link focus:outline underline">Alpine Linux</a>
			is supported. It is a popular, secure, lightweight distro with small image size that reduces build time and attack surface. Support for Ubuntu and official PHP images is in the roadmap.
		</div>
	{/if}
</div>
