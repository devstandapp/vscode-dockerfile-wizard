	...
	{#if $preferenceExpanded}
		<div class="inline-flex items-center mt-1">
			<label class="input-checkbox cursor-pointer">
				<input type="checkbox" id="cbx-jsBuildStage" bind:checked={$jsBuildStage} />
				{@html iconCheckbox}
			</label>
			<label class="cursor-pointer pl-2" for="cbx-jsBuildStage">Build frontend with Node.js</label>
		</div>

		{#if $jsBuildStage}
			<div class="inline-flex items-center gx-2">
				<span>Node.js version</span>
				<select class="input-text pl-1 h-8 rounded" bind:value={$jsNodeImageTag}>
			...
			<div class="inline-flex items-center gx-2">
				...
				<select class="input-text pl-1 h-8 rounded mono-family" bind:value={$jsBuildCommand}>
			...
			<div>
				...
				<textarea class="input-text rounded mono-family lh-normal w-full" rows="2" bind:this={jsPathsForBuildTextareaElement} bind:value={$jsPathsForBuildText} />
			</div>
			...
			<div>
				...
				<textarea class="input-text rounded mono-family lh-normal w-full" rows="2" bind:this={jsOutPathsTextareaElement} bind:value={$jsOutPathsText} />
			</div>
		{/if}
	{/if}
</div>
