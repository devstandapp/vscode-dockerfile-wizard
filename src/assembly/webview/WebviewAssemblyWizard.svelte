	...
	{:else}
			...
			<div class="px-4 pt-8">
				<div class="inline-flex items-center">
					<label class="input-checkbox cursor-pointer">
						<input type="checkbox" id="ghaCreate" bind:checked={$ghaCreate} />
						{@html iconCheckbox}
					</label>
					<label class="cursor-pointer pl-2" for="ghaCreate">Also create GitHub Actions workflow file</label>
				</div>
			</div>
		{/if}
		...
		<div class="p-4 mb-8 inline-flex items-center gx-4">
			<button
				...
				on:click={() => {
					if ($saveEnabled) {
						extension.onWizardRequestedSave({ ghaCreate: $ghaCreate })
					}
				}}
			>
				Create Dockerfile
			...
			<button
				class="button button-secondary select-none"
				class:opacity-25={!$saveEnabled}
				class:cursor-not-allowed={!$saveEnabled}
				disabled={!$saveEnabled}
				on:click={() => {
					if ($saveEnabled) {
						extension.onWizardRequestedPreview()
					}
				}}
			>
				Preview
