...
export default async function(folder: vscode.WorkspaceFolder, rootDirectoryListing: [string, vscode.FileType][], excludePattern: string): Promise<ContainerLanguageReport> {
	...
	if (fileExists('.dockerignore', rootDirectoryListing)) {
		report.dockerIgnoreExistingText = await (
			vscode.workspace.fs.readFile(vscode.Uri.joinPath(folder.uri, '.dockerignore')).then(data => data.toString())
		)
	}

	report.dockerIgnoreProposedLines = [
		...await proposeDockerIgnoreFromGitIgnore(folder, rootDirectoryListing),
		...await proposeDockerIgnoreFromPopularIgnore(folder, rootDirectoryListing),
	].filter(unique)
...
async function proposeDockerIgnoreFromGitIgnore(folder: vscode.WorkspaceFolder, rootDirectoryListing: [string, vscode.FileType][] = []): Promise<string[]> {
	...
	const foundGitIgnoreFiles = (await vscode.workspace.findFiles(new vscode.RelativePattern(folder, '**/.gitignore'), excludePattern, 500))
		.filter(gitignoreUri => gitignoreUri.path != vscode.Uri.joinPath(folder.uri, '.gitignore').path)
	...
	await Promise.allSettled(
		foundGitIgnoreFiles.map(gitignoreUri => {
			...
			return vscode.workspace.fs.readFile(gitignoreUri)
...
async function proposeDockerIgnoreFromPopularIgnore(folder: vscode.WorkspaceFolder, rootDirectoryListing: [string, vscode.FileType][] = []): Promise<string[]> {
	...
	return Promise.resolve(
		[
			'.git', '.gitignore',
			'.vs', '.vscode', '.fleet', '.idea',
			'.env', '.env.*',
			'.project', '.classpath', '.settings', '.toolstarget',
			'*.*proj.user', '*.dbmdl', '*.jfm',
			'.dockerignore', 'docker-compose*', 'compose*', 'Dockerfile*',
			'bin', 'obj', 'charts',
			'node_modules', 'npm-debug.log',
			'secrets.dev.yaml', 'values.dev.yaml',
			'LICENSE',
