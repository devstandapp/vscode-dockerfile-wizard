...
export default async function (
	...
	containerReport: ContainerLanguageReport,
	phpReport: PhpLanguageReport,
	jsReport: JsLanguageReport | undefined = undefined
): Promise<void> {
	let phpDirectoriesPaths = [
		...
		.filter(unique)
		.sort()
	let phpFilesPaths = [...['artisan', 'server.php'].filter(fileListed(rootDirectoryListing))].filter(unique).sort()
	...
	if (jsReport) {
		let jsDirectoriesPaths = [...['public'].filter(directoryListed(rootDirectoryListing)), ...jsReport.pathsWithSources].filter(unique).sort()
		let jsFilesPaths = [...['vite.config.js', 'webpack.mix.js'].filter(fileListed(rootDirectoryListing))].filter(unique).sort()
