...
export default async function (folder: vscode.WorkspaceFolder, rootDirectoryListing: [string, vscode.FileType][], excludePattern: string): Promise<PhpLanguageReport> {
	...
	const composerJson = report.hasComposerJson ? await readJsonFile(vscode.Uri.joinPath(folder.uri, 'composer.json')) : undefined
	const composerJsonRequired = composerJson && typeof composerJson['require'] == 'object' ? Object.keys(composerJson['require']) : []
	const composerJsonDev = composerJson && typeof composerJson['require-dev'] == 'object' ? Object.keys(composerJson['require-dev']) : []

	report.phpVersionFromPackageJson = composerJson && composerJsonRequired.includes('php') ? maxVersion(composerJson['require']['php'].toString()) : undefined
	...
	const composerLock = report.hasComposerLock ? await readJsonFile(vscode.Uri.joinPath(folder.uri, 'composer.lock')) : undefined

	report.phpModulesFromRequiredPackages = composerLock && Array.isArray(composerLock['packages']) ? extractModulesListFromPkg(composerLock['packages']) : []

	report.phpModulesFromDevPackages = composerLock && Array.isArray(composerLock['packages-dev']) ? extractModulesListFromPkg(composerLock['packages-dev']) : []

	report.phpModulesSuggestedByRequiredPackages = composerLock && Array.isArray(composerLock['packages']) ? extractModulesListFromPkg(composerLock['packages'], 'suggest', true) : []
